...
_resolve_host_display() {
  local candidate=""
  # stale Xvfb from virtual screenshot tests (:99, :100, ...)
...
if ! [[ -S "/tmp/.X11-unix/X${HOST_DISPLAY#:}" ]]; then
  ...
  echo "hint: run from a graphical session; check: ls /tmp/.X11-unix/" >&2
  exit 1
fi

if ! [[ -f "${XAUTHORITY}" ]]; then
  echo "warning: XAUTHORITY file not found: ${XAUTHORITY}" >&2
fi
