...
def analyze_history(
    ...
    *,
    ...
    for session_dir in discover_session_dirs(root=base):
        ...
        events = iter_session_events(session_dir)
        for event in events:
    ...
    observe_artifacts = {
        ...
        "context_json": len(list(observe_dir.glob("*.context.json"))) if observe_dir.is_dir() else 0,
        "vql_json": len(list(observe_dir.glob("*.vql.json"))) if observe_dir.is_dir() else 0,
    }

    total_tasks_ok = sum(1 for task in index.tasks if task.ok)
    total_tasks_failed = sum(1 for task in index.tasks if not task.ok)
    total_events = sum(event_histogram.values())
