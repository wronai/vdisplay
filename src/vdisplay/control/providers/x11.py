...
class X11ControlProvider(ControlProvider):
    ...
    def snapshot(
        ...
        *,
        ...
        if window is None:
            raise VDisplayError(_snapshot_hint(app, self.display))

        snapshot = _window_to_snapshot(window, backend=self.name)
...
def _window_to_snapshot(window: dict[str, Any], *, backend: str) -> ControlSnapshot:
